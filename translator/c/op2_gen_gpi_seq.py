...
def op2_gen_gpi_seq(master, date, consts, kernels):
...
#
    ...
    code('')
    if grouped:
      code('int set_size = op_gpi_halo_exchanges_grouped(set, nargs, args, 1);')
    else:
      code('int set_size = op_gpi_halo_exchanges(set, nargs, args);')
...
#
    if ninds>0:
      ...
      IF('n==set->core_size')
      if grouped:
        code('op_gpi_waitall_grouped(nargs, args, 1);')
      else:
...
#        code('op_gpi_reduce(&<ARG>,('+typs[g_m]+'*)<ARG>.data);')
        if typs[g_m] == 'double': #need for both direct and indirect
          ...
          code('op_gpi_reduce_float(&<ARG>,('+typs[g_m]+'*)<ARG>.data);')
        elif typs[g_m] == 'int':
          code('op_gpi_reduce_int(&<ARG>,('+typs[g_m]+'*)<ARG>.data);')
        else:
...
##########################################################################
    if not os.path.exists('gpi_seq'):
        os.makedirs('gpi_seq')
    fid = open('gpi_seq/'+name+'_gpiseqkernel.cpp','w')
...
##########################################################################
  ...
  for nk in range(0,len(kernels)):
    code('#include "'+kernels[nk]['name']+'_gpiseqkernel.cpp"')
  master = master.split('.')[0]
  fid = open('gpi_seq/'+master.split('.')[0]+'_gpiseqkernels.cpp','w')
  fid.write('//\n// auto-generated by op2.py\n//\n\n')
  fid.write(file_text)
  fid.close()

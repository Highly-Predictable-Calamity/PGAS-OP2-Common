...
int op_gpi_halo_exchanges_prep(op_set set, int nargs, op_arg *args){
  int size = set->size;
  int direct_flag = 1;
  ...
  for (int n = 0; n < nargs; n++) {
    if (args[n].opt && args[n].argtype == OP_ARG_DAT) {
      if (args[n].map == OP_ID) {
        op_gpi_exchange_halo_prep(&args[n], exec_flag);
      } else {
...
int op_gpi_halo_exchanges_send(op_set set, int nargs, op_arg *args){
  int size = set->size;
  int direct_flag = 1;
  ...
  for (int n = 0; n < nargs; n++) {
    if (args[n].opt && args[n].argtype == OP_ARG_DAT) {
      if (args[n].map == OP_ID) {
        op_gpi_exchange_halo_send(&args[n], exec_flag);
      } else {
        // Check if dat-map combination was already done or if there is a
        // mismatch (same dat, diff map)
        int found = 0;
